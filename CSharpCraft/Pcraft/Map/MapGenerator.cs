...
{
    private static readonly F32 Half = F32.FromFloat(0.5f);
    ...
    internal static F32[,] Noise(int sx, int sy, F32 startScale, F32 scaleMod, int featStep, Random rng)
    {
        ...
        while (step > 1)
        {
            F32 cscal = (step == featStep) ? F32.FromInt(1) : scale;
    ...
    internal static int[,] CreateMapStep(int sx, int sy, int a, int b, int c, int d, int e, Random rng)
    {
        ...
        for (int i = 0; i <= sx; i++)
        {
            for (int j = 0; j <= sy; j++)
            {
                ...
                F32 di    = F32.Abs(F32.FromFloat((float)i / sx) - Half) * F32.FromInt(2);
                F32 dj    = F32.Abs(F32.FromFloat((float)j / sy) - Half) * F32.FromInt(2);
                ...
                int id = a;
                if (coast > F32.FromFloat(0.3f)) id = b;
                if (coast > F32.FromFloat(0.6f)) id = c;
                if (coast > F32.FromFloat(0.3f) && v2 > F32.FromFloat(0.5f)) id = d;
                if (id == c && v3 > F32.FromFloat(0.5f)) id = e;
