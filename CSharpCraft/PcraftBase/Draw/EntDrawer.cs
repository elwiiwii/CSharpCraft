    ...
    internal static void DrawEnt(Level level)
    {
        foreach (Entity e in level.Ent)
        {
            switch (e)
            {
                ...
                case TextPopupEntity popup:
                    Pico8.Pal();
                    PcraftServices.PrintB(popup.TextValue.Double.ToString("0"),
                        e.X.Double - 2, e.Y.Double - 4, popup.TextColor, PicoColor._01DarkBlue);
                    break;
