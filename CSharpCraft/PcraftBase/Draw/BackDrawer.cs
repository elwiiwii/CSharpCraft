...
internal static class BackDrawer
{
    internal static void DrawBack(WorldState state)
    {
        ...
        for (int i = ci; i <= ci + 8; i++)
        {
            for (int j = cj; j <= cj + 8; j++)
            {
                ...
                if (gr.Gr == 1) // sand (GrSand, GrFarm, GrWheat, GrPlant all have Gr==1)
                {
                    ...
                    Pico8.Mset(gi,     gj,     PcraftServices.RndSand(i,       j,       state) + sv);
                    Pico8.Mset(gi + 1, gj,     PcraftServices.RndSand(i + 0.5, j,       state) + sv);
                    Pico8.Mset(gi,     gj + 1, PcraftServices.RndSand(i,       j + 0.5, state) + sv);
                    Pico8.Mset(gi + 1, gj + 1, PcraftServices.RndSand(i + 0.5, j + 0.5, state) + sv);
                }
                else
                {
                    bool u = PcraftServices.Comp(i,     j - 1, gr, state);
                    bool d = PcraftServices.Comp(i,     j + 1, gr, state);
                    bool l = PcraftServices.Comp(i - 1, j,     gr, state);
                    bool r = PcraftServices.Comp(i + 1, j,     gr, state);

                    int b = gr == PcraftData.GrRock  ? 21
                          : gr == PcraftData.GrWater ? 26
                          : 16;

                    int tl = PcraftServices.CornerOffset(l, u, PcraftServices.Comp(i - 1, j - 1, gr, state), PcraftServices.RndCenter(i,       j,       state), innerCorner: 20, hOnly:  1, vOnly: 16, outer:  0);
                    int tr = PcraftServices.CornerOffset(r, u, PcraftServices.Comp(i + 1, j - 1, gr, state), PcraftServices.RndCenter(i + 0.5, j,       state), innerCorner: 19, hOnly:  1, vOnly: 18, outer:  2);
                    int bl = PcraftServices.CornerOffset(l, d, PcraftServices.Comp(i - 1, j + 1, gr, state), PcraftServices.RndCenter(i,       j + 0.5, state), innerCorner:  4, hOnly: 33, vOnly: 16, outer: 32);
                    int br = PcraftServices.CornerOffset(r, d, PcraftServices.Comp(i + 1, j + 1, gr, state), PcraftServices.RndCenter(i + 0.5, j + 0.5, state), innerCorner:  3, hOnly: 33, vOnly: 18, outer: 34);
        ...
        for (int i = ci - 1; i <= ci + 8; i++)
        {
            for (int j = cj - 1; j <= cj + 8; j++)
            {
                ...
                {
                    PcraftServices.WatAnim(i,       j,       state);
                    PcraftServices.WatAnim(i + 0.5, j,       state);
                    PcraftServices.WatAnim(i,       j + 0.5, state);
                    PcraftServices.WatAnim(i + 0.5, j + 0.5, state);
                }

                if (gr == PcraftData.GrWheat)
                {
                    F32 dd = PcraftServices.DirGetData(i, j, F32.Zero, state) - state.Time;
                    ...
                    PcraftServices.Spr4(i, j, gi, gj, 6, 6, 6, 6, 0, (x, y) => PcraftServices.RndSand(x, y, state));
                }

                if (gr.IsTree)
                {
                    if (gr.Pal != null) PcraftServices.SetPal(gr.Pal);
                    PcraftServices.Spr4(i, j, gi, gj, 64, 65, 80, 81, 0, (x, y) => PcraftServices.RndTree(x, y, state));
                    ...
                    if (Pico8.Mget(i + state.LevelX, j + 1) == 99)
                        PcraftServices.Spr4(i, j, gi, gj, 64, 65, 80, 81, 4, (x, y) => PcraftServices.RndTree(x, y, state));
                }
    ...
    {
        var gr2 = PcraftServices.GetDirectGr(i, j, state);
        return gr.Gr == gr2.Gr;
    }
    ...
    {
        F32 a = ((state.Time * F32.FromFloat(0.6f) + PcraftServices.WatVal(i, j, state) / F32.FromInt(100)) % F32.One) * F32.FromInt(19);
        if (a > F32.FromInt(16))
    ...
    internal static int RndCenter(double i, double j, WorldState state)
        => (F32.FloorToInt(PcraftServices.WatVal(i, j, state) / F32.FromInt(34)) + 18) % 20;

    internal static int RndSand(double i, double j, WorldState state)
        => F32.FloorToInt(PcraftServices.WatVal(i, j, state) / F32.FromInt(34)) + 1;

    internal static int RndTree(double i, double j, WorldState state)
        => F32.FloorToInt(PcraftServices.WatVal(i, j, state) / F32.FromInt(51)) * 32;
