...
internal class SettingsMenuObj
{
    ...
    private readonly float[] _hoverT = new float[SettingsMenuDefs.All.Length];
    ...
    internal void Update()
    {
        ...
        int bestIndex = -1;
        float bestDepth = 0f;
        for (int i = 0; i < SettingsMenuDefs.All.Length; i++)
        {
            if (!SettingsMenuDefs.All[i].IsEnabled) continue;
            float depth = MenuButtonGeometry.SignedDistanceToRoundedHull(
                cursor, SettingsMenuDefs.All[i].HullVertices, HoverRadius);
        ...
        for (int i = 0; i < SettingsMenuDefs.All.Length; i++)
        {
            MenuButtonDef btn = SettingsMenuDefs.All[i];
