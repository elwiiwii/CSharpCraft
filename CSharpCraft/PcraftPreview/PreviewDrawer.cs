...
internal static class PreviewDrawer
{
    ...
    private static int SpriteBase(int tileId) => tileId switch { 0 => 26, 3 => 21, _ => 16 };
    ...
    private static int RndCenter(double i, double j, double[,] rndWat)
        => (F32.FloorToInt(WatVal(i, j, rndWat) / F32.FromInt(34)) + 18) % 20;

    private static int RndSand(double i, double j, double[,] rndWat)
        => F32.FloorToInt(WatVal(i, j, rndWat) / F32.FromInt(34)) + 1;

    private static int RndTree(double i, double j, double[,] rndWat)
        => F32.FloorToInt(WatVal(i, j, rndWat) / F32.FromInt(51)) * 32;
