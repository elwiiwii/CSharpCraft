...
public sealed class SpawnFinderTests
{
    ...
    {
        FixedGrid g = new(0.5, gridSx, gridSy);
    ...
    {
        FixedGrid cur = new(1.0, gridSx, gridSy);
        FixedGrid cur2 = new(0.0, gridSx, gridSy);
        FixedGrid cur3 = new(1.0, gridSx, gridSy);
        FixedGrid cur4 = new(1.0, gridSx, gridSy);
    ...
    private static MapClassifier AllStoneClassifier(int gridSx = 64, int gridSy = 64)
    {
        FixedGrid cur = new(1.0, gridSx, gridSy);
        FixedGrid cur2 = new(0.0, gridSx, gridSy);
        FixedGrid cur3 = new(0.0, gridSx, gridSy); // v2 = 1.0 > 0.5 → d = 3
        FixedGrid cur4 = new(1.0, gridSx, gridSy);
        return new MapClassifier(cur, cur2, cur3, cur4, gridSx, gridSy, a: 0, b: 1, c: 2, d: 3, e: 4);
    }
    ...
    private static MapClassifier AllTreeClassifier(int gridSx = 64, int gridSy = 64)
    {
        FixedGrid cur = new(1.0, gridSx, gridSy);
        FixedGrid cur2 = new(0.0, gridSx, gridSy);
        FixedGrid cur3 = new(1.0, gridSx, gridSy); // v2 = 0 → no d override
        FixedGrid cur4 = new(0.0, gridSx, gridSy); // v3 = 1.0 > 0.5 → e = 4
        return new MapClassifier(cur, cur2, cur3, cur4, gridSx, gridSy, a: 0, b: 1, c: 2, d: 3, e: 4);
    }
    ...
    {
        Func<(int tileX, int tileY)?> act = () => SpawnFinder.FindSpawn(0L, classifier: null!, gridSx: 64, gridSy: 64);

        _ = act.Should().Throw<ArgumentNullException>().WithParameterName("classifier");
    }
    ...
    public void FindSpawn_ReturnsNull_WhenAllTilesAreBaseTile()
    {
        ...
        (int tileX, int tileY)? result = SpawnFinder.FindSpawn(0L, AllBaseClassifier(), gridSx: 64, gridSy: 64);

        _ = result.Should().BeNull(because: "id=0 (base/water) is not a valid spawn tile");
    }
    ...
    public void FindSpawn_ReturnsNull_WhenAllTilesAreStoneTile()
    {
        ...
        (int tileX, int tileY)? result = SpawnFinder.FindSpawn(0L, AllStoneClassifier(), gridSx: 64, gridSy: 64);

        _ = result.Should().BeNull(because: "id=3 (stone) is not a valid spawn tile");
    }
    ...
    public void FindSpawn_ReturnsNull_WhenAllTilesAreTreeTile()
    {
        ...
        (int tileX, int tileY)? result = SpawnFinder.FindSpawn(0L, AllTreeClassifier(), gridSx: 64, gridSy: 64);

        _ = result.Should().BeNull(because: "id=4 (tree) is not a valid spawn tile");
    }
    ...
    public void FindSpawn_ReturnsNull_AfterExhaustingAllCandidates_WithNoValidTile()
    {
        ...
        (int tileX, int tileY)? result = SpawnFinder.FindSpawn(999L, AllStoneClassifier(), gridSx: 64, gridSy: 64);

        _ = result.Should().BeNull(because: "500 candidates exhausted with no valid tile → null");
    }
    ...
    public void FindSpawn_ReturnsNonNull_WhenAllTilesAreId2(long masterSeed)
    {
        ...
        _ = SpawnFinder.FindSpawn(masterSeed, AllRareClassifier(), 64, 64)
    ...
    public void FindSpawn_ReturnsNonNull_WhenAllTilesAreId1()
    {
        ...
        FixedGrid cur = new(1.0);
        FixedGrid cur2 = new(0.0);
        FixedGrid cur3 = new(1.0);
        FixedGrid cur4 = new(1.0);
        MapClassifier classifier = new(cur, cur2, cur3, cur4, 64, 64,
            a: 0, b: 1, c: 1, d: 3, e: 4);

        _ = SpawnFinder.FindSpawn(0L, classifier, 64, 64)
    ...
    public void FindSpawn_ReturnedTile_IsWithinExpectedBounds()
    {
        ...
        (int tileX, int tileY)? result = SpawnFinder.FindSpawn(42L, AllRareClassifier(GridSx, GridSy), GridSx, GridSy);

        _ = result.Should().NotBeNull();
        _ = result!.Value.tileX.Should().BeInRange(minX, maxX,
            because: "spawn x must lie within [gridSx/8, gridSx*7/8]");
        _ = result.Value.tileY.Should().BeInRange(minY, maxY,
    ...
    public void FindSpawn_ReturnedTile_NeverExceedsOuterEighthMargin(long seed)
    {
        ...
        (int tileX, int tileY)? result = SpawnFinder.FindSpawn(seed, AllRareClassifier(GridSx, GridSy), GridSx, GridSy);

        _ = result.Should().NotBeNull();
        _ = result!.Value.tileX.Should().BeGreaterThanOrEqualTo(GridSx / 8,
            because: "spawn must not be placed in the outer 1/8 left margin");
        _ = result.Value.tileX.Should().BeLessThanOrEqualTo(GridSx * 7 / 8,
            because: "spawn must not be placed in the outer 1/8 right margin");
        _ = result.Value.tileY.Should().BeGreaterThanOrEqualTo(GridSy / 8,
            because: "spawn must not be placed in the outer 1/8 top margin");
        _ = result.Value.tileY.Should().BeLessThanOrEqualTo(GridSy * 7 / 8,
    ...
    public void FindSpawn_ReturnedTileId_IsInValidSpawnSet()
    {
        ...
        MapClassifier classifier = AllRareClassifier();
        (int tileX, int tileY)? result = SpawnFinder.FindSpawn(0L, classifier, 64, 64);
    ...
    public void FindSpawn_ReturnedTileId_IsInValidSpawnSet_AcrossMultipleSeeds()
    {
        ...
        for (int s = 0; s < 10; s++)
        {
            (int tileX, int tileY)? result = SpawnFinder.FindSpawn(s, classifier, 64, 64);
            _ = result.Should().NotBeNull(because: $"seed {s}: all tiles valid, a spawn must be found");
            int tileId = classifier.ClassifyTile(result!.Value.tileX, result.Value.tileY);
            _ = tileId.Should().BeOneOf(new[] { 1, 2 },
    ...
    public void FindSpawn_IsIdempotent_ForSameSeed()
    {
        ...
        (int tileX, int tileY)? first = SpawnFinder.FindSpawn(123L, classifier, 64, 64);
        (int tileX, int tileY)? second = SpawnFinder.FindSpawn(123L, classifier, 64, 64);
    ...
    public void FindSpawn_ProducesDifferentResults_ForDifferentSeeds()
    {
        ...
        int distinctCount = Enumerable.Range(0, 20)
            .Select(i => SpawnFinder.FindSpawn(i, classifier, 64, 64))
            .Distinct()
            .Count();

        _ = distinctCount.Should().BeGreaterThan(1,
    ...
    public void FindSpawn_WithRealNoiseGrids_ReturnsValidSpawnOrNull()
    {
        ...
        SeededNoiseGrid cur = new(42L, 64, 64, 64, 0.9, 0.2, 0);
        SeededNoiseGrid cur2 = new(42L, 64, 64, 8, 0.9, 0.4, 1);
        SeededNoiseGrid cur3 = new(42L, 64, 64, 8, 0.9, 0.3, 2);
        SeededNoiseGrid cur4 = new(42L, 64, 64, 4, 0.8, 1.1, 3);
        MapClassifier classifier = new(cur, cur2, cur3, cur4,
            gridSx: 64, gridSy: 64, a: 0, b: 1, c: 2, d: 3, e: 4);

        (int tileX, int tileY)? result = SpawnFinder.FindSpawn(42L, classifier, 64, 64);

        if (result is not null)
        {
            _ = result.Value.tileX.Should().BeInRange(64 / 8, 64 * 7 / 8,
                because: "returned x must be within spawn bounds");
            _ = result.Value.tileY.Should().BeInRange(64 / 8, 64 * 7 / 8,
                because: "returned y must be within spawn bounds");
            _ = classifier.ClassifyTile(result.Value.tileX, result.Value.tileY)
    ...
    public void FindSpawn_WithRealNoiseGrids_WhenResultIsNonNull_IsWithinBoundsAndValid(long seed)
    {
        ...
        SeededNoiseGrid cur = new(seed, 64, 64, 64, 0.9, 0.2, 0);
        SeededNoiseGrid cur2 = new(seed, 64, 64, 8, 0.9, 0.4, 1);
        SeededNoiseGrid cur3 = new(seed, 64, 64, 8, 0.9, 0.3, 2);
        SeededNoiseGrid cur4 = new(seed, 64, 64, 4, 0.8, 1.1, 3);
        MapClassifier classifier = new(cur, cur2, cur3, cur4,
            gridSx: 64, gridSy: 64, a: 0, b: 1, c: 2, d: 3, e: 4);

        (int tileX, int tileY)? result = SpawnFinder.FindSpawn(seed, classifier, 64, 64);
