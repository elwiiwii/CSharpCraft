...
public sealed class MapClassifierTests
{
    ...
    {
        var gridCur  = new FixedGrid(cur);
        var gridCur2 = new FixedGrid(cur2);
        var gridCur3 = new FixedGrid(cur3);
        var gridCur4 = new FixedGrid(cur4);
    ...
    {
        var grid = new SeededNoiseGrid(0L, 4, 4, 4, 0.9, 0.2, 0);
        var act = () => new MapClassifier(cur: null!, cur2: grid, cur3: grid, cur4: grid,
            gridSx: 4, gridSy: 4, a: 0, b: 1, c: 2, d: 3, e: 4);

        act.Should().Throw<ArgumentNullException>().WithParameterName("cur");
    }
    ...
    {
        var grid = new SeededNoiseGrid(0L, 4, 4, 4, 0.9, 0.2, 0);
        var act = () => new MapClassifier(cur: grid, cur2: null!, cur3: grid, cur4: grid,
            gridSx: 4, gridSy: 4, a: 0, b: 1, c: 2, d: 3, e: 4);

        act.Should().Throw<ArgumentNullException>().WithParameterName("cur2");
    }
    ...
    {
        var grid = new SeededNoiseGrid(0L, 4, 4, 4, 0.9, 0.2, 0);
        var act = () => new MapClassifier(cur: grid, cur2: grid, cur3: null!, cur4: grid,
            gridSx: 4, gridSy: 4, a: 0, b: 1, c: 2, d: 3, e: 4);

        act.Should().Throw<ArgumentNullException>().WithParameterName("cur3");
    }
    ...
    {
        var grid = new SeededNoiseGrid(0L, 4, 4, 4, 0.9, 0.2, 0);
        var act = () => new MapClassifier(cur: grid, cur2: grid, cur3: grid, cur4: null!,
            gridSx: 4, gridSy: 4, a: 0, b: 1, c: 2, d: 3, e: 4);

        act.Should().Throw<ArgumentNullException>().WithParameterName("cur4");
    }
    ...
    public void ClassifyTile_ReturnsA_WhenCoastIsAtOrBelowLowerThreshold()
    {
        ...
        var classifier = MakeClassifier(cur: 0.5, cur2: 0.5, cur3: 0.5, cur4: 0.5);

        classifier.ClassifyTile(2, 2).Should().Be(0, because: "coast=0 is below the 0.3 threshold → base tile a");
    }
    ...
    public void ClassifyTile_ReturnsA_WhenCoastIsExactlyAtLowerThreshold()
    {
        ...
        classifier.ClassifyTile(0, 0).Should().Be(0,
    ...
    public void ClassifyTile_ReturnsB_WhenCoastIsBetweenThresholds()
    {
        ...
        // Expects tile b = 1.
        var classifier = MakeClassifier(cur: 0.6, cur2: 0.5, cur3: 0.6, cur4: 0.6);
        // v = |0.6-0.5| = 0.1, v2 = 0, v3 = 0; dist at (2,2)=0 → coast = 0.4

        classifier.ClassifyTile(2, 2).Should().Be(1, because: "coast=0.4 ∈ (0.3,0.6] with v2≤0.5 → sand (b)");
    }
    ...
    public void ClassifyTile_ReturnsC_WhenCoastExceedsUpperThreshold()
    {
        ...
        var classifier = MakeClassifier(cur: 0.7, cur2: 0.5, cur3: 0.7, cur4: 0.7);
        // v = |0.7-0.5| = 0.2 → coast = 0.8

        classifier.ClassifyTile(2, 2).Should().Be(2, because: "coast=0.8 > 0.6 with v3≤0.5 → rare tile c");
    }
    ...
    public void ClassifyTile_ReturnsD_WhenCoastAboveLowerThresholdAndV2ExceedsHalf()
    {
        ...
        var classifier = MakeClassifier(cur: 0.6, cur2: 0.5, cur3: 0.05, cur4: 0.6);

        classifier.ClassifyTile(2, 2).Should().Be(3,
    ...
    public void ClassifyTile_ReturnsD_WhenCoastAboveUpperThresholdAndV2ExceedsHalf()
    {
        ...
        var classifier = MakeClassifier(cur: 0.7, cur2: 0.5, cur3: 0.1, cur4: 0.7);
    ...
    public void ClassifyTile_ReturnsE_WhenIdIsCAndV3ExceedsHalf()
    {
        ...
        var classifier = MakeClassifier(cur: 0.7, cur2: 0.5, cur3: 0.7, cur4: 0.1);

        classifier.ClassifyTile(2, 2).Should().Be(4,
    ...
    public void ClassifyTile_DoesNotReturnE_WhenIdIsDNotC()
    {
        ...
        var classifier = MakeClassifier(cur: 0.7, cur2: 0.5, cur3: 0.1, cur4: 0.1);
    ...
    public void ClassifyTile_ReturnsA_AtGridEdge_WhenDistanceSuppressionDominates()
    {
        ...
        classifier.ClassifyTile(0, 2).Should().Be(0,
    ...
    public void ClassifyTile_CanReturnNonBaseId_AtCenter_WhenDistanceIsZero()
    {
        ...
        classifier.ClassifyTile(2, 2).Should().NotBe(0,
    ...
    public void ClassifyTile_IsIdempotent_ForSameInputs()
    {
        ...
        var first  = classifier.ClassifyTile(2, 2);
        var second = classifier.ClassifyTile(2, 2);

        second.Should().Be(first, because: "ClassifyTile must be deterministic and idempotent");
    }
    ...
    public void ClassifyTile_RespectsCustomTileIds_InWaterfallCascade()
    {
        ...
        var grid = new SeededNoiseGrid(0L, 4, 4, 999, 0.0, 0.0, 0);
        var highCur  = new FixedGrid(0.7);
        var lowCur2  = new FixedGrid(0.5);
        var classifier = new MapClassifier(highCur, lowCur2, highCur, highCur,
            gridSx: 4, gridSy: 4, a: 10, b: 20, c: 99, d: 30, e: 40);

        classifier.ClassifyTile(2, 2).Should().Be(99,
    ...
    public void ClassifyTile_AllTilesOnSmallGrid_ReturnValueFromProvidedIdSet()
    {
        ...
        var cur  = new SeededNoiseGrid(1L, 4, 4, 4,  0.9, 0.2, 0);
        var cur2 = new SeededNoiseGrid(1L, 4, 4, 4,  0.9, 0.4, 1);
        var cur3 = new SeededNoiseGrid(1L, 4, 4, 8,  0.9, 0.3, 2);
        var cur4 = new SeededNoiseGrid(1L, 4, 4, 4,  0.8, 1.1, 3);
        ...
        for (int i = 0; i <= 4; i++)
        for (int j = 0; j <= 4; j++)
            classifier.ClassifyTile(i, j).Should().BeOneOf(validIds,
    ...
    public void ClassifyTile_DoesNotReturnD_WhenV2IsExactlyAtHalfThreshold()
    {
        ...
        var classifier = MakeClassifier(cur: 0.6, cur2: 0.5, cur3: 0.1, cur4: 0.6);

        classifier.ClassifyTile(2, 2).Should().Be(1,
    ...
    public void ClassifyTile_DoesNotReturnE_WhenV3IsExactlyAtHalfThreshold()
    {
        ...
        var classifier = MakeClassifier(cur: 0.7, cur2: 0.5, cur3: 0.7, cur4: 0.2);

        classifier.ClassifyTile(2, 2).Should().Be(2,
