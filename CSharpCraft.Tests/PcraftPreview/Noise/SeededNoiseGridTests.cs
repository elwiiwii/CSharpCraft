...
public sealed class SeededNoiseGridTests
{
    ...
    {
        Func<SeededNoiseGrid> act = () => new SeededNoiseGrid(masterSeed: 1L, gridSx: sx, gridSy: 4,
            featStep: 4, startScale: 0.9, scaleMod: 0.2, layerIndex: 0);

        _ = act.Should().Throw<ArgumentException>().WithParameterName("gridSx");
    }
    ...
    {
        Func<SeededNoiseGrid> act = () => new SeededNoiseGrid(masterSeed: 1L, gridSx: 4, gridSy: sy,
            featStep: 4, startScale: 0.9, scaleMod: 0.2, layerIndex: 0);

        _ = act.Should().Throw<ArgumentException>().WithParameterName("gridSy");
    }

    [Fact]
    public void Constructor_ThrowsArgumentException_WhenGridSxIsZero()
    {
        Func<SeededNoiseGrid> act = () => new SeededNoiseGrid(masterSeed: 1L, gridSx: 0, gridSy: 4,
            featStep: 4, startScale: 0.9, scaleMod: 0.2, layerIndex: 0);
    ...
    {
        Func<SeededNoiseGrid> act = () => new SeededNoiseGrid(masterSeed: 1L, gridSx: 4, gridSy: 0,
            featStep: 4, startScale: 0.9, scaleMod: 0.2, layerIndex: 0);
    ...
    {
        Func<SeededNoiseGrid> act = () => new SeededNoiseGrid(masterSeed: 1L, gridSx: -4, gridSy: 4,
            featStep: 4, startScale: 0.9, scaleMod: 0.2, layerIndex: 0);
    ...
    {
        Func<SeededNoiseGrid> act = () => new SeededNoiseGrid(masterSeed: 1L, gridSx: sx, gridSy: sy,
    ...
    {
        SeededNoiseGrid grid = new(masterSeed: 99L, gridSx: 4, gridSy: 4,
            featStep: 4, startScale: 0.9, scaleMod: 0.2, layerIndex: 0);

        _ = grid.GetValue(x, y).Should().Be(0.5, because: "corners are always initialised to 0.5 by the algorithm");
    }
    ...
    public void GetValue_ReturnsExactlyHalf_ForRightBoundaryColumn(int y)
    {
        ...
        SeededNoiseGrid grid = new(masterSeed: 42L, gridSx: 4, gridSy: 4,
            featStep: 4, startScale: 0.9, scaleMod: 0.2, layerIndex: 0);

        _ = grid.GetValue(4, y).Should().Be(0.5, because: "right-boundary non-corner cells are constant 0.5");
    }
    ...
    public void GetValue_ReturnsExactlyHalf_ForBottomBoundaryRow(int x)
    {
        ...
        _ = grid.GetValue(x, 4).Should().Be(0.5, because: "bottom-boundary non-corner cells are constant 0.5");
    }
    ...
    {
        SeededNoiseGrid grid = new(masterSeed: 77L, gridSx: 4, gridSy: 4,
            featStep: 4, startScale: 0.9, scaleMod: 0.2, layerIndex: 0);

        double first = grid.GetValue(2, 2);
        double second = grid.GetValue(2, 2);

        _ = second.Should().Be(first, because: "results must be memoized and stable");
    }
    ...
    public void GetValue_ReturnsSameResult_ForTwoGridsWithIdenticalParameters()
    {
        ...
        SeededNoiseGrid gridA = new(masterSeed: 123L, gridSx: 4, gridSy: 4,
            featStep: 4, startScale: 0.9, scaleMod: 0.2, layerIndex: 0);
        SeededNoiseGrid gridB = new(masterSeed: 123L, gridSx: 4, gridSy: 4,
            featStep: 4, startScale: 0.9, scaleMod: 0.2, layerIndex: 0);

        for (int x = 0; x <= 4; x++)
            for (int y = 0; y <= 4; y++)
                _ = gridA.GetValue(x, y).Should().Be(gridB.GetValue(x, y),
    ...
    public void GetValue_ProducesDistinctInteriorValues_AcrossManyDifferentSeeds()
    {
        ...
        List<double> interiorValues = Enumerable.Range(1, 30)
            .Select(s => new SeededNoiseGrid(s, 4, 4, 4, 0.9, 0.2, 0).GetValue(2, 2))
            .ToList();

        _ = interiorValues.Distinct().Should().HaveCountGreaterThan(1,
    ...
    public void GetValue_DiffersAcrossLayerIndices_ForInteriorCell()
    {
        ...
        List<double> layers = Enumerable.Range(0, 5)
            .Select(l => new SeededNoiseGrid(masterSeed, 4, 4, 4, 0.9, 0.2, l).GetValue(2, 2))
            .ToList();

        _ = layers.Distinct().Should().HaveCountGreaterThan(1,
    ...
    public void GetValue_ConstantCells_RemainsHalfRegardlessOfLayerIndex()
    {
        ...
        {
            SeededNoiseGrid grid = new(masterSeed: 1L, gridSx: 4, gridSy: 4,
                featStep: 4, startScale: 0.9, scaleMod: 0.2, layerIndex: layer);

            _ = grid.GetValue(0, 0).Should().Be(0.5, because: "corners are always 0.5 regardless of layer");
            _ = grid.GetValue(4, 0).Should().Be(0.5, because: "corners are always 0.5 regardless of layer");
        }
    }
    ...
    public void GetValue_AllCellsReturnHalf_WhenFeatStepNeverFiresAndStartScaleIsZero()
    {
        ...
        for (int x = 0; x <= 4; x++)
            for (int y = 0; y <= 4; y++)
                _ = grid.GetValue(x, y).Should().Be(0.5,
    ...
    public void GetValue_HorizontalMidpointCell_IsExactAverageOfParents_WhenStartScaleIsZero()
    {
        ...
        double parent0 = grid.GetValue(0, 0);   // = 0.5 (corner)
        double parent2 = grid.GetValue(2, 0);   // = 0.5 (no jitter case)
        double mid = grid.GetValue(1, 0);   // should equal (parent0 + parent2) / 2

        _ = mid.Should().Be((parent0 + parent2) / 2,
    ...
    public void GetValue_CenterCell_IsExactAverageOfFourParents_WhenStartScaleIsZero()
    {
        ...
        SeededNoiseGrid grid = new(masterSeed: 13L, gridSx: 4, gridSy: 4,
            featStep: 999, startScale: 0.0, scaleMod: 0.0, layerIndex: 0);

        _ = grid.GetValue(2, 2).Should().Be(0.5,
    ...
    public void GetValue_HorizontalMidpointCell_IsExactAverageOfParents_WhenOnlyFeatStepFiresAtHigherStep()
    {
        ...
        SeededNoiseGrid grid = new(masterSeed: 9L, gridSx: 4, gridSy: 4,
            featStep: 4, startScale: 0.0, scaleMod: 0.0, layerIndex: 0);

        double parent0 = grid.GetValue(0, 0);   // corner = 0.5
        double parent2 = grid.GetValue(2, 0);   // has featStep jitter
        double mid = grid.GetValue(1, 0);
    ...
    public void GetValue_AtFeatStepCells_ProducesDistinctValues_AcrossSeeds()
    {
        ...
        List<double> values = Enumerable.Range(1, 30)
            .Select(s => new SeededNoiseGrid(s, 4, 4, featStep: 4, 0.0, 0.0, 0).GetValue(2, 0))
            .ToList();

        _ = values.Distinct().Should().HaveCountGreaterThan(1,
    ...
    public void GetValue_AtFeatStepCell_DiffersFromNonFeatStepEquivalent_ForSameSeed()
    {
        ...
        {
            double withFeat = new SeededNoiseGrid(s, 4, 4, featStep: 4, 0.0, 0.0, 0).GetValue(2, 2);
            double withoutFeat = new SeededNoiseGrid(s, 4, 4, featStep: 999, 0.0, 0.0, 0).GetValue(2, 2);
            return withFeat != withoutFeat;
        });

        _ = anyDiffers.Should().BeTrue(
    ...
    public void GetValue_AllValues_AreInUnitRange_WhenCscalNeverExceedsOne()
    {
        ...
        SeededNoiseGrid grid = new(masterSeed: 321L, gridSx: 4, gridSy: 4,
            featStep: 4, startScale: 0.0, scaleMod: 0.0, layerIndex: 0);

        for (int x = 0; x <= 4; x++)
            for (int y = 0; y <= 4; y++)
                _ = grid.GetValue(x, y).Should().BeInRange(0.0, 1.0,
    ...
    public void GetValue_FullIslandGrid_AllValuesInUnitRange_WithTypicalIslandParameters()
    {
        ...
        SeededNoiseGrid grid = new(masterSeed: 1L, gridSx: 64, gridSy: 64,
            featStep: 64, startScale: 0.9, scaleMod: 0.2, layerIndex: 0);

        Random rng = new(0); // deterministic sampling
    ...
    public void GetValue_VerticalMidpointCell_IsExactAverageOfParents_WhenStartScaleIsZero()
    {
        ...
        SeededNoiseGrid grid = new(masterSeed: 55L, gridSx: 4, gridSy: 4,
            featStep: 999, startScale: 0.0, scaleMod: 0.0, layerIndex: 0);

        double parent0 = grid.GetValue(0, 0);  // corner = 0.5
        double parent2 = grid.GetValue(0, 2);  // vertical midpoint at step=4, no jitter = 0.5
        double mid = grid.GetValue(0, 1);
    ...
    public void GetValue_NestedCenterCell_ReflectsParentJitter_WhenFeatStepFiresAtLargerStep()
    {
        ...
        SeededNoiseGrid grid = new(masterSeed: 7L, gridSx: 4, gridSy: 4,
            featStep: 4, startScale: 0.0, scaleMod: 0.0, layerIndex: 0);

        double p00 = grid.GetValue(0, 0);  // corner = 0.5
        double p20 = grid.GetValue(2, 0);  // featStep jitter
        double p02 = grid.GetValue(0, 2);  // featStep jitter
        double p22 = grid.GetValue(2, 2);  // featStep jitter (center type at step=4)
        double center11 = grid.GetValue(1, 1);

        double expectedAvg = (p00 + p20 + p02 + p22) / 4.0;
        _ = center11.Should().BeApproximately(expectedAvg, precision: 1e-12,
