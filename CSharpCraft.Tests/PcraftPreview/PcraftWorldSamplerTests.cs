...
public sealed class PcraftWorldSamplerTests
{
    ...
    {
        SampleResult result = PcraftWorldSampler.Sample(1L, radius, forceCenterX: 32, forceCenterY: 32);

        int expectedSide = (2 * radius) + 1;
        _ = result.Tiles.GetLength(0).Should().Be(expectedSide,
            because: $"radius={radius}: Tiles first dimension must be 2×radius+1");
        _ = result.Tiles.GetLength(1).Should().Be(expectedSide,
    ...
    {
        SampleResult result = PcraftWorldSampler.Sample(0L, radius: 4, forceCenterX: 30, forceCenterY: 35);
    ...
    {
        SampleResult result = PcraftWorldSampler.Sample(7L, radius: 2, forceCenterX: fx, forceCenterY: fy);

        _ = result.CenterTileX.Should().Be(fx);
        _ = result.CenterTileY.Should().Be(fy);
    }
    ...
    public void Sample_CenterTile_EqualsSpawnTile_WhenNoCenterForcedAndSpawnFound(long seed)
    {
        ...
        {
            _ = result.CenterTileX.Should().Be(result.SpawnTileX,
                because: "CenterTile must equal the spawn tile when no center override is provided");
            _ = result.CenterTileY.Should().Be(result.SpawnTileY,
                because: "CenterTile must equal the spawn tile when no center override is provided");
        }
    ...
    public void Sample_SpawnTileXAndY_AreBothValidOrBothNegativeOne()
    {
        ...
        SampleResult result = PcraftWorldSampler.Sample(42L, radius: 4);

        bool xFound = result.SpawnTileX >= 0;
        bool yFound = result.SpawnTileY >= 0;
        _ = xFound.Should().Be(yFound,
    ...
    public void Sample_SpawnTileId_IsInValidSpawnSet_WhenSpawnFound(long seed)
    {
        ...
        MapClassifier classifier = new(cur, cur2, cur3, cur4, 64, 64, 0, 1, 2, 3, 4);

        _ = classifier.ClassifyTile(result.SpawnTileX, result.SpawnTileY)
    ...
    {
        SampleResult result = PcraftWorldSampler.Sample(0L, radius: 2, forceCenterX: 32, forceCenterY: 32);
    ...
    {
        SampleResult result = PcraftWorldSampler.Sample(5L, radius: 2, forceCenterX: 32, forceCenterY: 32);
    ...
    {
        SampleResult first = PcraftWorldSampler.Sample(13L, radius: 2, forceCenterX: 32, forceCenterY: 32);
        SampleResult second = PcraftWorldSampler.Sample(13L, radius: 2, forceCenterX: 32, forceCenterY: 32);

        for (int i = 0; i < 16; i++)
            for (int j = 0; j < 16; j++)
                _ = second.RndWat[i, j].Should().Be(first.RndWat[i, j],
    ...
    {
        SampleResult a = PcraftWorldSampler.Sample(1L, radius: 2, forceCenterX: 32, forceCenterY: 32);
        SampleResult b = PcraftWorldSampler.Sample(2L, radius: 2, forceCenterX: 32, forceCenterY: 32);
    ...
    {
        SampleResult result = PcraftWorldSampler.Sample(seed, radius: 4, forceCenterX: 32, forceCenterY: 32);
        ...
        for (int i = 0; i < side; i++)
            for (int j = 0; j < side; j++)
                _ = result.Tiles[i, j].Should().BeOneOf(valid,
    ...
    {
        SampleResult first = PcraftWorldSampler.Sample(99L, radius: 3, forceCenterX: 32, forceCenterY: 32);
        SampleResult second = PcraftWorldSampler.Sample(99L, radius: 3, forceCenterX: 32, forceCenterY: 32);

        int side = first.Tiles.GetLength(0);
        for (int i = 0; i < side; i++)
            for (int j = 0; j < side; j++)
                _ = second.Tiles[i, j].Should().Be(first.Tiles[i, j],
                    because: $"Tiles[{i},{j}] must be identical across repeated calls with the same seed");

        _ = second.SpawnTileX.Should().Be(first.SpawnTileX);
        _ = second.SpawnTileY.Should().Be(first.SpawnTileY);
        _ = second.CenterTileX.Should().Be(first.CenterTileX);
        _ = second.CenterTileY.Should().Be(first.CenterTileY);
    }

    [Fact]
    public void Sample_ProducesDifferentTiles_ForDifferentSeeds()
    {
        SampleResult a = PcraftWorldSampler.Sample(1L, radius: 4, forceCenterX: 32, forceCenterY: 32);
        SampleResult b = PcraftWorldSampler.Sample(2L, radius: 4, forceCenterX: 32, forceCenterY: 32);
    ...
    public void Sample_Tiles00_MatchesDirectClassification_AtRadius0()
    {
        ...
        SampleResult result = PcraftWorldSampler.Sample(Seed, radius: 0, forceCenterX: Cx, forceCenterY: Cy);
        ...
        _ = result.Tiles[0, 0].Should().Be(classifier.ClassifyTile(Cx, Cy),
    ...
    public void Sample_TileSlice_CornerAndCenterMatchDirectClassification()
    {
        ...
        SampleResult result = PcraftWorldSampler.Sample(Seed, radius: R, forceCenterX: Cx, forceCenterY: Cy);

        SeededNoiseGrid cur = new(Seed, 64, 64, 64, 0.9, 0.2, 0);
        SeededNoiseGrid cur2 = new(Seed, 64, 64, 8, 0.9, 0.4, 1);
        SeededNoiseGrid cur3 = new(Seed, 64, 64, 8, 0.9, 0.3, 2);
        SeededNoiseGrid cur4 = new(Seed, 64, 64, 4, 0.8, 1.1, 3);
        MapClassifier classifier = new(cur, cur2, cur3, cur4, 64, 64, 0, 1, 2, 3, 4, generateHole: true);

        _ = result.Tiles[0, 0].Should().Be(classifier.ClassifyTile(Cx - R, Cy - R),
            because: "Tiles[0,0] must be the top-left corner of the slice at (Cx-R, Cy-R)");
        _ = result.Tiles[R, R].Should().Be(classifier.ClassifyTile(Cx, Cy),
            because: "Tiles[R,R] (center index) must be the center tile classification");
        _ = result.Tiles[2 * R, 2 * R].Should().Be(classifier.ClassifyTile(Cx + R, Cy + R),
