...
public sealed class PcraftMathTests
{
    ...
    public void GetLen_IsSafe_ForZeroVector()
    {
        ...
        _ = PcraftMath.GetLen(F32.Zero, F32.Zero).Float.Should().BeApproximately(MathF.Sqrt(0.001f), precision: 0.001f);
    }
    ...
    public void GetLen_ReturnsCorrectLength_ForUnitVector()
    {
        ...
        _ = PcraftMath.GetLen(F32.One, F32.Zero).Float.Should().BeApproximately(MathF.Sqrt(1.001f), Eps);
    }

    [Fact]
    public void GetLen_ReturnsCorrectLength_ForDiagonalVector()
    {
        _ = PcraftMath.GetLen(F32.FromInt(3), F32.FromInt(4)).Float.Should().BeApproximately(MathF.Sqrt(25.001f), Eps);
    }
    ...
    public void GetInvLen_IsReciprocalOfGetLen()
    {
        ...
        _ = PcraftMath.GetInvLen(x, y).Should().Be(F32.One / PcraftMath.GetLen(x, y));
    }
    ...
    {
        // Should not throw
        Func<F32> act = () => PcraftMath.GetInvLen(F32.Zero, F32.Zero);
    ...
    {
        _ = PcraftMath.GetRot(F32.FromFloat(dx), F32.FromFloat(dy)).Float.Should().BeApproximately(expected, Eps);
    }
    ...
    public void GetRot_TreatsZeroDy_AsDyGreaterOrEqualZero()
    {
        ...
        _ = PcraftMath.GetRot(F32.Zero, F32.Zero).Float.Should().BeApproximately(0.75f, Eps);
    }
    ...
    {
        (int flipX, int flipY) = PcraftMath.Mirror(F32.FromFloat(rot));
        _ = flipX.Should().Be(expectedFlipX);
        _ = flipY.Should().Be(expectedFlipY);
    }
    ...
    public void UpRot_SmoothlylLerps_WhenDifferenceIsSmall()
    {
        ...
        _ = PcraftMath.UpRot(grot: F32.FromFloat(0.8f), rot: F32.FromFloat(0.5f)).Float.Should().BeApproximately(0.62f, Eps);
    }
    ...
    public void UpRot_WrapsAroundSeam_WhenCurrentGreaterThanTarget()
    {
        ...
        // ((0.98%1)+1)%1 = 0.98
        _ = PcraftMath.UpRot(grot: F32.FromFloat(0.1f), rot: F32.FromFloat(0.9f)).Float.Should().BeApproximately(0.98f, Eps);
    }
    ...
    public void UpRot_WrapsAroundSeam_WhenCurrentLessThanTarget()
    {
        ...
        // ((0.02%1)+1)%1 = 0.02
        _ = PcraftMath.UpRot(grot: F32.FromFloat(0.9f), rot: F32.FromFloat(0.1f)).Float.Should().BeApproximately(0.02f, Eps);
    }

    [Fact]
    public void UpRot_ResultIsInZeroToOneRange()
    {
        F32 result = PcraftMath.UpRot(grot: F32.FromFloat(0.95f), rot: F32.FromFloat(0.05f));
        _ = result.Float.Should().BeGreaterThanOrEqualTo(0f).And.BeLessThan(1f);
    }
    ...
    public void UpRot_ReturnsTarget_WhenCurrentEqualsTarget()
    {
        ...
        _ = PcraftMath.UpRot(grot: F32.FromFloat(0.4f), rot: F32.FromFloat(0.4f)).Float.Should().BeApproximately(0.4f, Eps);
    }
