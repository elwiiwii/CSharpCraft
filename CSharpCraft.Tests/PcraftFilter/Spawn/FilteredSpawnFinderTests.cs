...
public sealed class FilteredSpawnFinderTests
{
    ...
    {
        return new(new FixedGrid(1.0), new FixedGrid(0.0),
    ...
    {
        return new(new FixedGrid(0.5), new FixedGrid(0.5),
    ...
    private sealed class PatchClassifier : MapClassifier
    {
        ...
        {
            return _rockPatch.Contains(i, j) ? 3 : 2;  // 3=Rock, 2=Grass
        }
    }
    ...
    {
        Func<(int tileX, int tileY)?> act = () => FilteredSpawnFinder.FindSpawn(0L, null!, new BiasLayers(), 64, 64, null);
    ...
    {
        Func<(int tileX, int tileY)?> act = () => FilteredSpawnFinder.FindSpawn(0L, AllGrass(), null!, 64, 64, null);
        _ = act.Should().Throw<ArgumentNullException>().WithParameterName("biases");
    }
    ...
    {
        (int tileX, int tileY)? result = FilteredSpawnFinder.FindSpawn(42L, AllGrass(), new BiasLayers(), 64, 64, null);
    ...
    {
        (int tileX, int tileY)? result = FilteredSpawnFinder.FindSpawn(42L, AllWater(), new BiasLayers(), 64, 64, null);
    ...
    public void FindSpawn_ReturnsSpawnInsideZone_WhenZoneCoversSpawnRange()
    {
        ...
        SpawnConstraintFilter filter = new([zone]);

        (int tileX, int tileY)? result = FilteredSpawnFinder.FindSpawn(42L, AllGrass(), new BiasLayers(), 64, 64, filter);

        _ = result.Should().NotBeNull();
        (int x, int y) = result!.Value;
        _ = zone.Contains(x, y).Should().BeTrue();
    }
    ...
    public void FindSpawn_SearchesUnionOfZones_WhenMultipleAllowedZones()
    {
        ...
        RectangleZone zone1 = new(10, 10, 16, 16);  // 10..25 × 10..25
        RectangleZone zone2 = new(38, 38, 16, 16);  // 38..53 × 38..53
        SpawnConstraintFilter filter = new([zone1, zone2]);

        (int tileX, int tileY)? result = FilteredSpawnFinder.FindSpawn(42L, AllGrass(), new BiasLayers(), 64, 64, filter);

        _ = result.Should().NotBeNull();
        (int x, int y) = result!.Value;
        _ = (zone1.Contains(x, y) || zone2.Contains(x, y)).Should().BeTrue();
    }
    ...
    public void FindSpawn_ReturnsSpawnNearCluster_WhenProximityConstraintMet()
    {
        ...
        //                = [23, 40] × [23, 40].
        PatchClassifier classifier = new(new RectangleZone(28, 28, 8, 8));
        TileClusterProximity proximity = new(MaxDistance: 5, ClusterTileId: 3, MinClusterSize: 1);
        SpawnConstraintFilter filter = new([new RectangleZone(0, 0, 64, 64)], proximity);

        (int tileX, int tileY)? result = FilteredSpawnFinder.FindSpawn(42L, classifier, new BiasLayers(), 64, 64, filter);

        _ = result.Should().NotBeNull();
        (int x, int y) = result!.Value;
        _ = (x >= 23 && x <= 40 && y >= 23 && y <= 40).Should().BeTrue();
    }
    ...
    public void FindSpawn_ReturnsNull_WhenNoClusterMeetsMinimumSize()
    {
        ...
        PatchClassifier classifier = new(new RectangleZone(30, 30, 2, 2));
        TileClusterProximity proximity = new(MaxDistance: 5, ClusterTileId: 3, MinClusterSize: 20);
        SpawnConstraintFilter filter = new([new RectangleZone(0, 0, 64, 64)], proximity);
    ...
    public void FindSpawn_CallsOnSpawnFallbackApplied_WhenNoInitialSpawnInZone()
    {
        ...
        RectangleZone zone = new(16, 16, 32, 32);
        SpawnConstraintFilter filter = new([zone]);
        FilteredSpawnFinderCapturingSink sink = new();

        _ = FilteredSpawnFinder.FindSpawn(42L, AllWater(), new BiasLayers(), 64, 64, filter, sink);

        _ = sink.SpawnFallbacks.Should().Equal([filter]);
    }
    ...
    public void FindSpawn_ReturnsFallbackSpawn_AfterBiasApplicationMakesZoneSpawnable()
    {
        ...
        (int tileX, int tileY)? result = FilteredSpawnFinder.FindSpawn(42L, AllWater(), new BiasLayers(), 64, 64, filter);
