...
### Game Loop — Frame-Rate-Driven (PSharp8)
- All update and draw callbacks fire **once per game frame** (60fps via `FNAGame.IsFixedTimeStep`). The `fps` parameter in `RegisterUpdate`/`RegisterDraw` is ignored — the accumulator system has been removed.
...
- **Input latching**: `InputManager` polls hardware every frame (60fps). `_pressedThisFrame` and `_consumedSincePress` latch until `ConsumePressedFlags()` is called by `SceneManager` after the primary callback once per frame.
...
- **Testing** frame-rate-driven behavior: all enabled, non-paused registrations fire on every `InternalUpdate`/`InternalDraw` call. `ConsumePressedFlags` is called exactly once per frame after the primary callback.
